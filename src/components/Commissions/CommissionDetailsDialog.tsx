...
function DetailRow({
    ...
    return (
        <div className={className ?? 'space-y-1'}>
            <p
                className={`text-xs md:text-sm font-semibold text-gray-400 ${labelClassName}`}
            >
                {label}
            </p>
...
export function CommissionDetailsDialog({
    ...
    const attendantName =
        item.personal_order?.customer_reengagement?.attendant?.user?.name ??
        'NA';
    const attendantLogin =
        item.personal_order?.customer_reengagement?.attendant?.user?.login ??
        'NA';
    const reengagementDate = item.personal_order?.customer_reengagement
        ?.created_at
            ...
            <DialogContent className="max-w-2xl border border-white/10 bg-surface p-0 text-on-surface">
                <DialogHeader className="border-b border-white/5 px-5 py-4">
                    <DialogTitle className="text-base">
                        Transação #{item.id ?? 'NA'}
                    </DialogTitle>
                    <DialogDescription className="text-xs text-on-surface-variant">
                        Pedido {item.order_id ?? 'NA'} em{' '}
                        {item.personal_order?.payment_date
                            ? formatDateTime(item.personal_order.payment_date)
                            : 'NA'}
                    </DialogDescription>
                </DialogHeader>
                ...
                <div className="space-y-5 px-5 py-4">
                    <div className="grid gap-4 sm:grid-cols-2">
                        <DetailRow
                            label="Cliente"
                            value={`${customerName} (${customerLogin})`}
                        />
                        <DetailRow
                            valueClassName="font-normal"
                            label="Atendente"
                            value={`${attendantName} (${attendantLogin})`}
                        />
                        <DetailRow
                            label="Valor do pedido"
                            value={formatCurrency(
                                item.personal_order?.value ?? 0,
                            )}
                        />
                        <DetailRow
                            label="Comissão"
