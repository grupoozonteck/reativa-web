...
export function CustomerLatestOrderCard({
    order,
    orderStatusCollection,
}: CustomerLatestOrderCardProps) {
    const [expanded, setExpanded] = useState(false);
    const status = orderStatusStyleMap[order.status] || orderStatusStyleMap[1];
    const statusLabel =
        orderStatusCollection[String(order.status)] || 'Desconhecido';
    const deliveryStatus =
        typeof order.delivery_status === 'number'
    ...
    return (
        <div
            className="solid-card p-5 animate-fade-in"
        ...
        >
            <div className="flex flex-col gap-3 mb-4 sm:flex-row sm:items-center sm:justify-between">
                ...
                <Badge
                    className={cn('text-xs border gap-1.5 w-fit', status.color)}
                >
            ...
            <div className="grid grid-cols-2 gap-4 mb-5 md:grid-cols-4">
                ...
                <div>
                    ...
                    <p
                        className={cn(
                            'text-sm font-medium',
                            deliveryStatus?.color || 'text-on-surface-variant',
                        )}
                    >
            ...
            {order.personal_order_items &&
                order.personal_order_items.length > 0 && (
                    <div className="border-t border-border/40 mt-2 pt-2">
                        ...
                        >
                            <span className="flex items-center gap-2">
                                ...
                                {expanded
                                    ? 'Ocultar itens'
                                    : `Ver ${order.personal_order_items.length} ${order.personal_order_items.length === 1 ? 'item' : 'itens'} do pedido`}
                            </span>
                            {expanded ? (
