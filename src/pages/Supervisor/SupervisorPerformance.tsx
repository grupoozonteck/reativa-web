...
export default function SupervisorPerformance() {
    ...
    const { data, isLoading, isFetching, refetch } = useQuery({
        ...
        queryFn: () =>
            teamService.getSupervisorPerformance({
    ...
    const totalSales =
        summary?.total_sales ?? members.reduce((acc, m) => acc + m.sales, 0);
    const totalReengagements =
        summary?.total_reengagements ??
        members.reduce((acc, m) => acc + m.total_reengagements, 0);
    const totalRevenue =
        summary?.total_revenue ??
        members.reduce((acc, m) => acc + Number(m.revenue ?? 0), 0);
    const avgConversion =
        summary?.conversion ??
        (members.length > 0
            ? Math.round(
                  members.reduce((acc, m) => acc + m.conversion, 0) /
                      members.length,
              )
            : 0);
